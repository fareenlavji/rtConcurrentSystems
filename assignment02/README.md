...
```mermaid
  sequenceDiagram
    ...
    C->>R: UDP DatagramPacket (requestEvent: bytes)
    R->>S: Forward request packet (no mods)
    S->>S: Update state
    S-->>R: UDP DatagramPacket (responseEvent: bytes)
    R-->>C: Forward response packet (no mods)
```
#### 2.1.2. Server Response Flow
```mermaid
  sequenceDiagram
  autonumber
  participant S as server.app
...
#### 2.2.1. Domain Model (Game Environment)
```mermaid
classDiagram
  direction LR
